...
- **`geosort_algorithm.py`** — `GeoSortAlgorithm(QgsProcessingAlgorithm)` that wraps `geosort_core` for use in the Processing Toolbox, the graphical modeler, and headless PyQGIS. Defines the 18 sort criteria as an enum (`_CRITERIA_KEYS` / `_CRITERIA_LABELS`).
